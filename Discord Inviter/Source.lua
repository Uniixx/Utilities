...
Inviter.Join = function(invite)
    ...
    local success, inviteData =
        pcall(
        function()
            return HS:JSONDecode(
                Exploit.Request(
                    {Url = "https://ptb.discord.com/api/invites/" .. removeFromString(invite, "/"), Method = "GET"}
                ).Body
            )
        end
    )
...
Inviter.Prompt = function(invite)
    ...
    local success, inviteData =
        ...
        function()
            return HS:JSONDecode(
                Exploit.Request(
                    {Url = "https://ptb.discord.com/api/invites/" .. removeFromString(inviteCode, "/"), Method = "GET"}
                ).Body
            )
        end
    )
    ...
    connections.joinEntered =
        Prompt.Frame.Join.MouseEnter:Connect(
        function()
            TS:Create(Prompt.Frame.Join, TweenInfo.new(0.25), {BackgroundColor3 = Color3.fromRGB(75, 85, 200)}):Play()
        end
    )

    connections.joinLeft =
        Prompt.Frame.Join.MouseLeave:Connect(
        function()
    ...
    connections.joinClick =
        ...
        function()
            task.spawn(Inviter.Join, removeFromString(inviteCode, "/"))
    ...
    connections.ignoreEntered =
        Prompt.Frame.Ignore.MouseEnter:Connect(
        function()
            Prompt.Frame.Ignore.Line.Visible = true
        end
    )

    connections.ignoreLeft =
        Prompt.Frame.Ignore.MouseLeave:Connect(
        function()
            Prompt.Frame.Ignore.Line.Visible = false
        end
    )

    connections.ignoreClick =
        Prompt.Frame.Ignore.Activated:Connect(
        function()
            disconnect(Prompt)
            toggle(Prompt, false)
        end
